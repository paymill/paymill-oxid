...
class paymill_order extends paymill_order_parent
{
    ...
    public function execute()
    {
        ...
        $logger = oxRegistry::get('paymill_logger');

        if (strpos($nextStep,'thankyou') !== false && $this->isPaymillPayment()) {
            $orderId = $oxSession->getVariable('sess_challenge');
            $transactionId = $oxSession->getVariable('paymillPgTransId');
            ...
            $privateKey = trim(
                oxRegistry::getConfig()->getShopConfVar('PAYMILL_PRIVATEKEY')
            );
